  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border rounded-lg p-4 flex flex-wrap items-end gap-4">
        ...
        <Button size="sm" className="h-8 text-sm" onClick={handleApply}>Apply</Button>
      </div>
              ...
              <tbody>
                {items.map((r: any) => (
                  <tr key={r.id} className="border-b last:border-0 hover:bg-muted/30">
                    <td className="px-4 py-2 font-mono text-xs">{truncateUUID(r.id || r.request_id)}</td>
                    <td className="px-4 py-2">{r.sender}</td>
                    <td className="px-4 py-2">{r.request_type}</td>
                    <td className="px-4 py-2 text-muted-foreground">{r.created_at ? new Date(r.created_at).toLocaleDateString() : "—"}</td>
                    <td className="px-4 py-2"><StatusBadge status={r.status} /></td>
                    <td className="px-4 py-2">{r.reviewed_by || "—"}</td>
                    <td className="px-4 py-2 text-muted-foreground">{r.completed_at ? new Date(r.completed_at).toLocaleDateString() : "—"}</td>
                  </tr>
                ))}
