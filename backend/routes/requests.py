...
from models import Request, RequestItem, VendorLookup
...
def get_request(request_id: UUID, db: Session = Depends(get_db)):
    ...
    # For change_existing requests, enrich items with vendor lookup current values
    if req.request_type == "change_existing" and req.vendor_number:
        vendor = db.query(VendorLookup).filter(VendorLookup.vendor_number == req.vendor_number).first()
        if vendor:
            vendor_data = {
                "VENDOR_NAME": vendor.vendor_name,
                "VENDOR_ACCT_GROUP": vendor.acct_group,
                ...
                "BANK_ACCT_NUMBER": vendor.bank_acct_number,
                "BANK_ACCT_HOLDER": vendor.bank_acct_holder,
